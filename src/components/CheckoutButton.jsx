...
import { paymentForArt , fetchArtsByArtId} from '../services/ApiService';
...
const CheckoutButton = ({ amount, currency,currencySymbol, email, productname,artId,userId }) => {
  const handleCheckout = async () => {
    ...
    const artDetails = await fetchArtsByArtId(artId);
    if(artDetails.data.art_status != 3){
      ...
      showNotification({
        title: "Unavailable",
        message: "Apologies, this artwork has just been sold to another collector.",
        type: "danger",
      });
    return;  
    }     
